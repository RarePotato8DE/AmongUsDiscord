...
namespace AmongUsCapture
{
    ...
    internal class GameMemReader
    {
        ...
        public static readonly IGameOffsets _gameOffsets = Settings.GameOffsets;
        private bool exileCausesEnd;

        private bool shouldReadLobby = false;
        public static IntPtr GameAssemblyPtr = IntPtr.Zero;
        ...
        public void RunLoop()
        {
            ...
            while (true)
            {
                if (!ProcessMemory.IsHooked)
                {
                    ...
                    {
                        if (runFirst && Program.debug_mode)
                        {
                            Console.ForegroundColor = ConsoleColor.Yellow;
                            Console.WriteLine("Searching for Among Us..");
                            Console.ResetColor();
                            runFirst = false;
                        }
                        Thread.Sleep(1000);
                    ...
                    if (Program.debug_mode)
                    {
                        ...
                        Console.WriteLine("Hooked Among Us");
                        Console.ResetColor();
                    }
